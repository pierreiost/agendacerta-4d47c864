...
BEGIN
  ...
  IF p_segment = 'custom' THEN
    ...
    WHERE venue_id = p_venue_id AND status_simple = 'completed'
  ...
  SELECT COALESCE(SUM(gt), 0) INTO v_current_balance FROM (
    ...
    SELECT total AS gt FROM service_orders WHERE venue_id = p_venue_id
      AND ((order_type = 'simple' AND status_simple = 'completed') OR (order_type = 'complete' AND status_complete = 'delivered'))
  ) r;
  ...
  SELECT jsonb_agg(
    jsonb_build_object('day', to_char(days.d, 'DD/MM'), 'projected_balance',
      v_current_balance + COALESCE(SUM(fr.amt) OVER (ORDER BY days.d), 0) - COALESCE(SUM(fp.amt) OVER (ORDER BY days.d), 0)
    ) ORDER BY days.d
  ) INTO v_projection
  FROM days
  ...
  IF p_segment = 'custom' THEN
    ...
    SELECT COALESCE(jsonb_agg(
      jsonb_build_object('day_label', to_char(dy.day_date, 'DD/MM'), 'day_name', to_char(dy.day_date, 'Dy'), 'count', COALESCE(sub.cnt, 0), 'total_value', COALESCE(sub.total, 0))
