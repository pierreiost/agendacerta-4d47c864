...
export function CrmColumn({ column, leads, onDeleteLead, onClickLead }: CrmColumnProps) {
  return (
    <div className="flex-1 min-w-[200px] flex flex-col rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md overflow-hidden">
      {/* Header */}
        ...
        {(provided, snapshot) => (
          ...
          >
            {leads.map((lead, index) => (
              <CrmLeadCard key={lead.id} lead={lead} index={index} onDelete={onDeleteLead} onClick={() => onClickLead(lead)} />
            ))}
