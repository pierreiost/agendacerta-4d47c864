...
export function BeautyBookingSheet({
  ...
  const startTime = booking ? parseISO(booking.start_time) : null;
  ...
  const getProfessionalName = (professional: { display_name: string | null; profile: { full_name: string } | null } | null) => {
    ...
    <>
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent className="w-[400px] sm:w-[540px] flex flex-col p-0 bg-background/80 backdrop-blur-xl border-l border-border/50 shadow-2xl">
          {!booking ? (
            /* Loading state inside the single SheetContent */
      ...
      {/* Auxiliary dialogs - only render when booking exists */}
      {booking && (
        <>
          <AddProductDialog
            open={addProductOpen}
